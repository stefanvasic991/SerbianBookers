...
import com.easyswitch.serbianbookers.gantogram.view.BarView;
import com.easyswitch.serbianbookers.gantogram.view.CalendarView;
import com.easyswitch.serbianbookers.gantogram.view.ScaleView;
...
public class GanttTableAdapter extends BaseTableAdapter {
    ...
    private int nx = 0;
    private int dx = 40; //dayAxisX-width

    private List<Room> rooms = null;
    private List<Reservation> reservations = null;
    private int ny = 0;
    private int dy = 40; //dayAxisY-height
    ...
    public GanttTableAdapter(AppCompatActivity compatActivity, GanttData ganttData) {
        ...
        this.rooms = ganttData.getTasks();
        this.reservations = ganttData.getMilestones();

        int taskCnt = rooms.size();
        ny = taskCnt * 2;

        // Determining the date to display
        DateTime projectStartDate = DateTime.parse(project.getStartDate(), DateTimeFormat.forPattern("yyyy-MM-dd"));
        int s = projectStartDate.getDayOfWeek() - 1;
        if (s == 0) {
            minDate = projectStartDate;
        } else {
            minDate = projectStartDate.minusDays(0);
            if (minDate.getDayOfMonth() > 50) {
                minDate = minDate.minusWeeks(1);
            }
        }

        DateTime projectEndDate = DateTime.parse(project.getEndDate(), DateTimeFormat.forPattern("yyyy-MM-dd"));
...
//        scale.setMilestones(reservations);
...
//            task_bar.setTaskAndStatus(true);
    ...
    public int getHeight(int row) {
        ...
        switch (row) {
            ...
            case 0:
                ...
                int dispHeight = size.y - Math.round(760 * scale);
