    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        webApiClient.getCalDetails(c).observe(this, new Observer<Calendar>() {
            ...
            public void onChanged(Calendar calendar) {
                ...
                roomID = calendar.getAvailabilityList().get(1).getId();

                if (calendar.getAvailabilityList() != null) {
                    calendarList.clear();
                    calendarList.addAll(calendar.getAvailabilityList().get(1).getData());
                    calendarAdapter.notifyDataSetChanged();
                } else {
                    List<AvailabilityData> tmpList = new ArrayList<>();
                    tmpList.addAll(calendar.getAvailabilityList().get(1).getData());
                }
            }
        });
